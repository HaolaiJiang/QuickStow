...
export class SyncService {
  ...
  private _isSyncEnabled: boolean = false;
  ...
  async syncToCloud(): Promise<void> {
    ...
    for (const id of pendingIds) {
      ...
      if (entry) {
        try {
          await this.saveToFirestore(entry as SyncEntry);
   ...
   * Requirements: 4.1
   */
  private async saveToFirestore(entry: SyncEntry): Promise<void> {
    if (!this.userId) {
   ...
   * Requirements: 4.2
   */
  private async loadFromFirestore(): Promise<SyncEntry[]> {
    if (!this.userId) {
      return [];
    }
  ...
  mergeEntries(localEntries: Entry[], cloudEntries: SyncEntry[]): SyncEntry[] {
    const entryMap = new Map<string, SyncEntry>();

    // Add local entries to map
    for (const entry of localEntries) {
      const syncEntry: SyncEntry = {
        ...entry,
        updatedAt: (entry as SyncEntry).updatedAt || entry.timestamp,
      };
   ...
   */
  private async uploadLocalOnlyEntries(
    localEntries: Entry[],
   ...
   */
  private updateSyncMeta(meta: Partial<SyncMeta>): void {
   ...
   */
  private getPendingChanges(): string[] {
   ...
   */
  private addPendingChange(entryId: string): void {
   ...
   */
  private removePendingChange(entryId: string): void {
    const pending = this.getPendingChanges();
